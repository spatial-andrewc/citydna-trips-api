...
class TripsGenerator(Resource):
    ...
    def get(self, origin, destination, m_per_second, access_token):
        """
        ...
        """
        response = self._use_router(origin=origin, destination=destination, access_token=access_token)
    ...
    def _use_router(self, origin, destination, access_token):
        """
        :param origin: a comma seperated lon,lat pair describing trip origin
        :param destination: a comma seperated lon,lat pair describing trip destination
...
api.add_resource(TripsGenerator, '/trips/origin/<string:origin>/destination/<string:destination>/m_per_second/<int:m_per_second>/access_token/<string:access_token>')
